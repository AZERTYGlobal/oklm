...
| E10, E11 | `hid` accepts values HUT 1.7 marks non-physical (`00`–`03`) or reserved (`E8`–`FFFF`); `code` accepts any `[A-Za-z0-9]+` rather than the W3C enumeration, and nothing enforces "only one `Backslash`". | Linted by `validators/validate.py --strict` since 2026-09-15: `hid` outside `04`-`E7` is an error, `code` must be one of the 172 values of the W3C Recommendation (2025-04-22, `validators/w3c_codes.py`), and one `code` names one key. The 0.1 schema itself is unchanged. |

...
| **E21** | **Silent levels.** OKLM says "a level with no output is simply omitted"; LDML says keystrokes are ignored where no layer matches, unless a layer `other` exists. The two are not the same, and the schema cannot express `other`. | **Defect, measured on the committed exports:** a key without output at a level is dropped from that layer's `<row>`, so the row is shorter than the form's scan codes and every following key shifts one position. Example: `azerty-global.ldml.xml`, layer `altR`, row D has 8 keys for 12 scan codes. LDML's `gap` key is the intended fix. |
